...
import { LoginDto, SendOtpDto, VerifySmsOtpDto } from "src/dto/user.dto";
...
export class UsersController {
    ...
    @Post('send-otp/:id')
    async sendOtp(@Body() body: SendOtpDto, @Param() param: any) {
        await this.smsService.sendOtp(body.phoneNumber, param.id)
    }
    @Get('verify-otp/:id')
    async verifyOtp(@Body() body: VerifySmsOtpDto, @Param() param: any) {
